...
export default function Services({
  ...
  return (
    <section
      id="servicios"
      className="section-divider relative overflow-hidden bg-[linear-gradient(to_bottom,#f7fbff,#edf4ff)] px-4 py-24 sm:px-6 lg:px-8"
    >
      ...
      <div className="relative mx-auto max-w-7xl">
        <SectionReveal className="mb-16 text-center">
          <span className="mb-4 inline-flex items-center gap-2 rounded-full border border-blum-blue/10 bg-white/75 px-4 py-2 text-sm font-semibold text-blum-blue shadow-sm backdrop-blur">
          ...
          {t.items.map((service, index) => {
            ...
            return (
              <SectionReveal key={service.title} delay={index * 0.08}>
                <article
                  className={`group relative h-full overflow-hidden rounded-[30px] border p-8 backdrop-blur-xl transition duration-500 hover:-translate-y-2 ${
                    featured
                ...
                >
                  ...
                  <div className="relative">
                    {featured && (
                      <div className="mb-5 inline-flex rounded-full border border-blum-blue/10 bg-white/70 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-blum-blue shadow-sm">
                        Principal
                      </div>
                    )}
