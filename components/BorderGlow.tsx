...
const BorderGlow = forwardRef<HTMLDivElement, BorderGlowProps>(function BorderGlow(
  ...
  const getEdgeProximity = useCallback(
    (element: HTMLElement, x: number, y: number) => {
      ...
      const activationBand = Math.max(edgeSensitivity, 1);

      return Math.min(Math.max(1 - distanceToEdge / activationBand, 0), 1);
    },
    [edgeSensitivity]
  );
  ...
  const hasEdgeContact = edgeProximity > 0.001;
  const isVisible = sweepActive || (isHovered && hasEdgeContact);
  const borderOpacity = isVisible ? easeOutCubic(edgeProximity) : 0;
  const glowOpacity = isVisible ? easeOutCubic(edgeProximity) : 0;
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative z-[1] flex min-h-0 flex-col overflow-auto rounded-[inherit]",
          contentClassName
        )}
      >
        {children}
